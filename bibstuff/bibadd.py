...
try:
    input = raw_input  #handle Python 2
except NameError:
    pass
...
def make_entry(choosetype='', options=False, extras=False):
    ...
    if 'A' in fields or 'Æ' in fields:
        entry['author'] = input("author(s)? ").strip()
    if 'E' in fields or 'Æ' in fields:
        entry['editor'] = input("editor(s)? ").strip()
    if 'Y' in fields:
        entry['year'] = input("year? ").strip()
    if 'T' in fields:
        entry['title'] = input("title? ").strip()
    if 'b' in fields:
        entry['booktitle'] = input("booktitle? ").strip()
    if 'e' in fields:
        entry['edition'] = input("edition? (E.g., 2nd)  ").strip()
    if 'c' in fields:
        entry['chapter'] = input("chapter? ").strip()
    if 'j' in fields: #:TODO: journal key handling (all lower only?)
        entry['journal'] = input("journal name? ").strip()
    if 'v' in fields:
        entry['volume'] = input("volume? ").strip()
    if 'n' in fields:
        entry['number'] = input("number? ").strip()
    if 'm' in fields: #:TODO: macro handling
        entry['month'] = input("month? ").strip()
    if 'p' in fields:
        entry['pages'] = input("pages? ").strip()
    if 'P' in fields:
        entry['publisher'] = input("Publisher? ").strip()
    if 'a' in fields:
        entry['address'] = input("address? ").strip()
    if 'h' in fields:
        entry['howpublished'] = input("howpublished? ").strip()
    if 'I' in fields:
        entry['institution'] = input("Institution? ").strip()
    if 'O' in fields:
        entry['organization'] = input("Organization? ").strip()
    if 'S' in fields:
        entry['school'] = input("School? ").strip()
    if 's' in fields:
        entry['series'] = input("series? ").strip()
    if 't' in fields:
        entry['type'] = input("type? (E.g., Working Paper) ").strip()
    if 'i' in fields:
        entry['isbn'] = input("isbn? ").strip()
    if 'u' in fields:
        entry['url'] = input("url? ").strip()
    if 'k' in fields:
        entry['key'] = input("key (*not* citekey)? ").strip()
    if not citekey:
